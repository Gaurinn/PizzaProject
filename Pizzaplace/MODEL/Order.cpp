...
ostream& operator <<(ostream& outs, const Order& order){
    outs << order.nameOfCustomer << "," << order.phoneNumberOfCustomer << ",";
    return  outs;
}
 ...
 //       pizzamenu add_pizzaMenuToOrder(PizzaMenu pizzamenu);
   //     extrafood add_ExtraFoodToOrder(ExtraFood extrafood);
